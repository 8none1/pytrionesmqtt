    ...
    def handleNotification(self, cHandle, data):
        if cHandle == 12:
            ...
            if data[0] == "0x66" and data[1] == "0x4" and data[11] == "0x99":
                ...
                logger(f"{self.mac}    Sending: {json_status}")
                send_mqtt(self.mqtt_client, json_status)
            else:
                logger(f"{mac}    Didn't understand the response data.")
        else:
            logger(f"{mac}    Got a different handle: {cHandle}")
...
def mqtt_message_received(client, userdata, message):
    global WORK_LIST
    logger(f" * Adding request to queue:  {message.payload.decode('utf-8')}")
    if message.topic == mqtt_subscription_topic:
        ...
        if "mac" in request.keys() and "completed" not in request.keys():
            ...
            if request["completed"] == True and request["mac"] in WORK_LIST.keys():
                logger(f" * Cancelling remaining work for device {request['mac']}")
